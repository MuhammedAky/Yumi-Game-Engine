...
namespace yumi {
    ...
    void Model::loadMesh(uint32_t index) {
        ...
        loadedMeshes.push_back(new Mesh(vertices, indices, textures));
    }

    void Model::traverseNode(uint32_t nextNode, glm::mat4 matrix) {
    ...
    std::vector<unsigned char> Model::getData() {
        ...
        std::string bytesText = ReadFile((fileDir + uri).c_str());
        ...
        for (uint32_t i = 0; i < JSON["images"].size(); i++) {
            ...
            if (!skip) {
                if (texPath.find("baseColor") != std::string::npos || texPath.find("diffuse") != std::string::npos) {
                    Texture* texture = new Texture((fileDir + texPath).c_str(), "diffuse", loadedTextures.size());
                    ...
                    Texture* texture = new Texture((fileDir + texPath).c_str(), "specular", loadedTextures.size());
                    textures.push_back(texture);
                    loadedTextures.push_back(texture);
                    loadedTextureNames.push_back(texPath);
                }
